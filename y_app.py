...
def channel_streamlit():
    ...
    result=streamlit.selectbox("Select",chsql_list,index=None)
...
if streamlit.button("SQL"):
    ...
    ch_id='''select Channel_Id from channels; '''
    mycursor.execute(ch_id)
    ch_list=mycursor.fetchall()
    mydb.commit()
    
    
    cids_list = list(map(lambda x: x[0], ch_list))
 
    # selecting the row from the streamlit using channel_select function
    
    if selected_row['Channel_Id'] in cids_list:
        streamlit.success("Channel Details are in mysql ")
        
    else:
        Table=tables()
        streamlit.success(Table)
show_table=streamlit.radio("SELECT THE VIEW",("","VIDEOS","PLAYLISTS","COMMENTS"))
